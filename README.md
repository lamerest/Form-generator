...
После, идет поле "inputs", в нем содержится список объектов, поля которых аналогичны атрибутам
тега "input" и на основе каждого будет создан блок с input по указанным параметрам:
```json
...
На выходе получится такой блок:
```html
<div>
    <label for="first_name">Имя</label>
    <input id="first_name" type="text" required="" placeholder="Введите имя...">
</div>
```
...
### SUBMIT
Кнопку input[type=submit], которая будет обрабатывать форму следует определять следующим образом:
```json
{
  ...
  "submit": {
...
```
- Поле "submit" должно содержать в объекте поле "text"
- Нельзя переопределять onclick и class атрибуты кнопки 
- 
...
## ИТОГ
В случае, если что-то еще не понятно, то все вышеперечисленные возможности (+валидатор и сбор статистики) можно посмотреть на имеющемся в репозитории примере.